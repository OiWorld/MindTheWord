...
LIT.Replacement = {
  ...
  initiateParagraphLevelTranslation: function() {
    ...
    chrome.extension.sendRequest({
      ...
      // get paragraph-level translations
      LIT.Translation.translateParagraphList(that.replaced_paragraphs, function(response) {
        //console.log('RESPONSE: ', response); throw 'e';
  ...
  interpolateParagraphsFromResponse: function() {
    ...
    for(var i in this.api_response) {
      console.log(this.replaced_paragraphs[i])
      var rp_index = 0;
...
//      $('.gti-match-paragraph-'+i).text('').html(this.finalPassMatchDecorator(this.replaced_paragraphs[i], this.api_response[i]));
  ...
  getAllTextNodes: function(parent_selector) {
    var
      textNodes = [],
      whitespace = /^\s*$/,
      invalid_tags = /^(a|script|meta|head|noscript|link|style|a|embed|iframe|form|label|option|fieldset|input|select)$/i,
      taggy_text = /^\s*(<.+>.+<\/.+>)\s*$/,
      taggy_text2 = /^\s*(<.+\/>)\s*$/,
      node;
    ...
    function getTextNodes(node) {
      ...
      if (node.nodeType == 3) {
        if(!whitespace.test(node.nodeValue) && !taggy_text.test(node.nodeValue) && !taggy_text2.test(node.nodeValue)) {
          textNodes.push(node);
        }
...
LIT.Replacement.Event = {
  ...
  onMouseover: function(e) {
    ...
    get('playSounds', function(doPlaySounds) { 
      //console.log(translation.text(), doPlaySounds);
      if(doPlaySounds) {
        //console.log('doPlaySounds!!');
        timeOut = setTimeout( function() {
          //console.log('timeout hit!', LIT.language);
          chrome.extension.sendRequest({
            action: "playAudio",
