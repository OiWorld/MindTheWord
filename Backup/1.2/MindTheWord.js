...
function deepHTMLReplacement(node, tMap, iTMap){
  ...
  else if (node.nodeType == Node.ELEMENT_NODE && node.tagName != 'TEXTAREA' && node.tagName != 'INPUT' && node.tagName != 'SCRIPT') {
...
function processTranslations(translationMap, userDefinedTMap) { 
  ...
  for (w in translationMap) {
    if (w != translationMap[w] && translationMap[w] != "" && !userDefinedTMap[w]) {
...
function main() {
  ...
  chrome.extension.sendRequest({getOptions : "Give me the options chosen by the user..." }, function(response) {
      var activation = response.activation;
      var translationProbability = response.translationProbability;
      var minimumSourceWordLength = response.minimumSourceWordLength;
      var translatedWordStyle = response.translatedWordStyle;
      var userDefinedTranslations = response.userDefinedTranslations;

      if (activation == "true") {
          insertCSS(translatedWordStyle);
