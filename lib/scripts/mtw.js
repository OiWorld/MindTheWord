...
export class ContentScript {
  ...
  translateDeep(paragraph, filteredTMap, iMap) {
    for (let i in paragraph.childNodes) {
      if (paragraph.childNodes[i].nodeType === 3) {
        if(!/^\s*$/.test(paragraph.childNodes[i].textContent)){
          if((this.srcLang == yandexLanguages.Chinese)||(this.srcLang == bingLanguages["Chinese Simplified"])||(this.srcLang == bingLanguages["Chinese Traditional"])){4
  ...
  deepHTMLReplacement(node, tMap, iTMap) {
    ...
    } else if (node.nodeType === Node.ELEMENT_NODE && badTags.indexOf(node.tagName) <= -1 && node.className!='mtwTranslatedWord') {
  ...
  replaceAll(text, translationMap) {
    if(text.includes('class="mtwTranslatedWord"')||text.includes('<span')||text.includes('<a'))
      return text;
    var rExp = '';
