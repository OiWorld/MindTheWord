...
export class ContentScript {
  ...
  processTranslations(translationMap, userDefinedTMap) {
    ...
    if (Object.keys(filteredTMap).length !== 0) {
      ...
      if (this.oneWordTranslation) {
        ...
        for (var i = 0; i < anchors.length; i++) {
          this.deepHTMLReplacement(anchors[i], filteredTMap, this.invertMap(filteredTMap));
        }
        for (var i = 0; i < divs.length; i++) {
          this.deepHTMLReplacement(divs[i], filteredTMap, this.invertMap(filteredTMap));
        }
      }
    }
  ...
  replaceAll(text, translationMap) {
    if(text.includes('class="mtwTranslatedWord"'))
      return text;
    var rExp = '';
