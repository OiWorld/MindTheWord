...
export class ContentScript {
  ...
  processTranslations(translationMap, userDefinedTMap) {
    ...
    //number of words translated by each service in the current month
    let month = ['January', 'February', 'March', 'April', 'May', 'June', 'July', 'August', 'September', 'October', 'November', 'December'];
    let d = new Date();
    let currentMonth = month[d.getMonth()];
    if (currentMonth in this.stats['translatorWiseWordCount']){
      this.stats['translatorWiseWordCount'][currentMonth][this.translator] += numberOfTranslatedWords;
    }
    else{
    ...
    for (let i = 0; i < translatedWords.length; i++) {
      translatedWords[i].addEventListener('click', this.toggleElement, false);
    }
  };

  toggleElement(event, self) {
