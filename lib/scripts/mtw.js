...
export class ContentScript {
    ...
    getAllWords(ngramMin, ngramMax) {
        var countedWords = {},
            nparagraphs = document.querySelectorAll('p,div,a');
        console.log('Getting words from all ' + nparagraphs.length + ' paragraphs');
        console.time('someFunction');
        var j = 0;
        var paragraphs = [];
        for (var i = 0; i < nparagraphs.length; i++) {
            if (this.isInViewport(nparagraphs[i])) {
                paragraphs.push(nparagraphs[i]);
            }
        }
