...
import { CommonWords } from './services/commonWords';
...
export class ContentScript {
  ...
  initialize(res) {
    ...
    this.custom_icon = [];
    this.custom_icon.push(chrome.extension.getURL('assets/img/volume.png'));
    this.custom_icon.push(chrome.extension.getURL('assets/img/bookmark.png'));
  ...
  translate() {
    ...
    if (this.userDefinedOnly === true) {
      ...
      if (this.cwAvailable === true) {
        ...
        var cwHandle = new CommonWords(this.srcLang);
        testConnection(cwHandle.url);
  ...
  processTranslations(translationMap, userDefinedTMap, cwMatch) {
    ...
    for (let i = 0; i < paragraphs.length; i++) {
      ...
      for (let i = 0; i < translatedWords.length; i++) {
        ...
        hoverHTML += '<img class="mtwMarkAsLearnt" data-original="' + translatedWords[i].getAttribute('data-original') + '" src="' + this.custom_icon[1] + '">';
        hoverHTML += '<img class="mtwVisualHint" data-translated="' + translatedWords[i].getAttribute('data-translated') + '" src="' + this.custom_icon[2] + '">';
    ...
    for (let i = 0; i < paragraphs.length; i++) {
      var speakButtonsList = paragraphs[i].querySelectorAll('.mtwSpeak');
      if (speakButtonsList.length) {
        for (let i = 0; i < speakButtonsList.length; i++) {
          speakButtonsList[i].addEventListener('click', function() {
      ...
      var learntButtonList = paragraphs[i].querySelectorAll('.mtwMarkAsLearnt');
      if (learntButtonList.length) {
        for (let i = 0; i < learntButtonList.length; i++) {
          learntButtonList[i].addEventListener('click', function() {
      ...
      var visualHintButtonList = paragraphs[i].querySelectorAll('.mtwVisualHint');
      if (visualHintButtonList.length) {
        var targetLanguage = this.targetLanguage;
        for (let i = 0; i < visualHintButtonList.length; i++) {
          visualHintButtonList[i].addEventListener('click', function() {
