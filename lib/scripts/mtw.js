...
export class ContentScript {
   ...
   * @param {boolean} dynamicallyLoaded - flag to check if dynamic content
   */
  translate(dynamicallyLoaded) {
    chrome.storage.local.get(null, (res) => {
      if (res.activation === true) {
        ...
        if (blacklistWebsiteReg.test(document.URL) && res.blacklist !== '()') {
          ...
          var countedWords = this.getAllWords(this.ngramMin, this.ngramMax, dynamicallyLoaded);
          var filteredWords;
   ...
   */
  getAllWords(ngramMin, ngramMax, dynamicallyLoaded) {
    var countedWords = {}, paragraphs;
    if(dynamicallyLoaded){
      paragraphs = dynamicallyLoadedPtags;
    } else {
...
MTWTranslator.translate(false);

var observer = new MutationSummary({
  callback: handleDynamicChanges,
  queries: [{ element: 'p' }]
});

var dynamicallyLoadedPtags = [];
function handleDynamicChanges(summaries){
  var dynamicChanges = summaries[0];
  dynamicallyLoadedPtags = [];
  var i=0;
  dynamicChanges.added.forEach(function(newEl) {
    dynamicallyLoadedPtags.push(newEl);
  });
  MTWTranslator.translate(true);
}
